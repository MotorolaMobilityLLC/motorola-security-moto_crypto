...
static const u32 moto_pc2[1024] = {
...
#define ROUND(L, R, A, B, K, d)					\
        ...
        L ^= MOTO_S8[0xff & B];		A &= 0x3f3f3f3f;	\
        L ^= MOTO_S6[0xff & (B >> 8)];	B >>= 16;		\
        L ^= MOTO_S7[0xff & A];					\
        L ^= MOTO_S5[0xff & (A >> 8)];	A >>= 16;		\
        L ^= MOTO_S4[0xff & B];					\
        L ^= MOTO_S2[0xff & (B >> 8)];				\
        L ^= MOTO_S3[0xff & A];					\
        L ^= MOTO_S1[0xff & (A >> 8)];
 ...
 *
 */
unsigned long moto_des_ekey(u32 *pe, const u8 *k)
{
    /* K&R: long is at least 32 bits */
    unsigned long a, b, c, d, w;
    const u32 *pt = moto_pc2;

    d = k[4]; d &= 0x0e; d <<= 4; d |= k[0] & 0x1e; d = moto_pc1[d];
    c = k[5]; c &= 0x0e; c <<= 4; c |= k[1] & 0x1e; c = moto_pc1[c];
    b = k[6]; b &= 0x0e; b <<= 4; b |= k[2] & 0x1e; b = moto_pc1[b];
    a = k[7]; a &= 0x0e; a <<= 4; a |= k[3] & 0x1e; a = moto_pc1[a];

    pe[15 * 2 + 0] = DES_PC2(a, b, c, d); d = moto_rs[d];
    pe[14 * 2 + 0] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[13 * 2 + 0] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[12 * 2 + 0] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[11 * 2 + 0] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[10 * 2 + 0] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[ 9 * 2 + 0] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[ 8 * 2 + 0] = DES_PC2(d, a, b, c); c = moto_rs[c];
    pe[ 7 * 2 + 0] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[ 6 * 2 + 0] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[ 5 * 2 + 0] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[ 4 * 2 + 0] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[ 3 * 2 + 0] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[ 2 * 2 + 0] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[ 1 * 2 + 0] = DES_PC2(c, d, a, b); b = moto_rs[b];
    pe[ 0 * 2 + 0] = DES_PC2(b, c, d, a);

    /* Check if first half is weak */
    w  = (a ^ c) | (b ^ d) | (moto_rs[a] ^ c) | (b ^ moto_rs[d]);

    /* Skip to next table set */
    pt += 512;

    d = k[0]; d &= 0xe0; d >>= 4; d |= k[4] & 0xf0; d = moto_pc1[d + 1];
    c = k[1]; c &= 0xe0; c >>= 4; c |= k[5] & 0xf0; c = moto_pc1[c + 1];
    b = k[2]; b &= 0xe0; b >>= 4; b |= k[6] & 0xf0; b = moto_pc1[b + 1];
    a = k[3]; a &= 0xe0; a >>= 4; a |= k[7] & 0xf0; a = moto_pc1[a + 1];

    /* Check if second half is weak */
    w |= (a ^ c) | (b ^ d) | (moto_rs[a] ^ c) | (b ^ moto_rs[d]);

    pe[15 * 2 + 1] = DES_PC2(a, b, c, d); d = moto_rs[d];
    pe[14 * 2 + 1] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[13 * 2 + 1] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[12 * 2 + 1] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[11 * 2 + 1] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[10 * 2 + 1] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[ 9 * 2 + 1] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[ 8 * 2 + 1] = DES_PC2(d, a, b, c); c = moto_rs[c];
    pe[ 7 * 2 + 1] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[ 6 * 2 + 1] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[ 5 * 2 + 1] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[ 4 * 2 + 1] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[ 3 * 2 + 1] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[ 2 * 2 + 1] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[ 1 * 2 + 1] = DES_PC2(c, d, a, b); b = moto_rs[b];
 ...
 *
 */
static void moto_dkey(u32 *pe, const u8 *k)
{
    /* K&R: long is at least 32 bits */
    ...
    pe[ 0 * 2] = DES_PC2(a, b, c, d); d = moto_rs[d];
    pe[ 1 * 2] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[ 2 * 2] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[ 3 * 2] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[ 4 * 2] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[ 5 * 2] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[ 6 * 2] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[ 7 * 2] = DES_PC2(d, a, b, c); c = moto_rs[c];
    pe[ 8 * 2] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[ 9 * 2] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[10 * 2] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[11 * 2] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[12 * 2] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[13 * 2] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[14 * 2] = DES_PC2(c, d, a, b); b = moto_rs[b];
    ...
    pe[ 0 * 2 + 1] = DES_PC2(a, b, c, d); d = moto_rs[d];
    pe[ 1 * 2 + 1] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[ 2 * 2 + 1] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[ 3 * 2 + 1] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[ 4 * 2 + 1] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[ 5 * 2 + 1] = DES_PC2(d, a, b, c); c = moto_rs[c]; b = moto_rs[b];
    pe[ 6 * 2 + 1] = DES_PC2(b, c, d, a); a = moto_rs[a]; d = moto_rs[d];
    pe[ 7 * 2 + 1] = DES_PC2(d, a, b, c); c = moto_rs[c];
    pe[ 8 * 2 + 1] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[ 9 * 2 + 1] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[10 * 2 + 1] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[11 * 2 + 1] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[12 * 2 + 1] = DES_PC2(c, d, a, b); b = moto_rs[b]; a = moto_rs[a];
    pe[13 * 2 + 1] = DES_PC2(a, b, c, d); d = moto_rs[d]; c = moto_rs[c];
    pe[14 * 2 + 1] = DES_PC2(c, d, a, b); b = moto_rs[b];
 ...
 *
 */
static int moto_des3_ede_setkey(struct crypto_tfm *tfm, const u8 *key,
        unsigned int keylen)
{
    const u32 *K = (const u32 *)key;
    struct moto_des3_ede_ctx *dctx = crypto_tfm_ctx(tfm);
    ...
    moto_des_ekey(expkey, key); 
    expkey += DES_EXPKEY_WORDS; key += DES_KEY_SIZE;
    moto_dkey(expkey, key); 
    expkey += DES_EXPKEY_WORDS; key += DES_KEY_SIZE;
    moto_des_ekey(expkey, key);
...
static void moto_des3_ede_encrypt(struct crypto_tfm *tfm, u8 *dst, 
        const u8 *src)
{
    struct moto_des3_ede_ctx *dctx = crypto_tfm_ctx(tfm);
...
static void moto_des3_ede_decrypt(struct crypto_tfm *tfm, u8 *dst, 
        const u8 *src)
{
    struct moto_des3_ede_ctx *dctx = crypto_tfm_ctx(tfm);
...
static int moto_ecb_des3_ede_operate(struct blkcipher_desc *desc,
        ...
        unsigned int nbytes, void (*fn)(struct crypto_tfm *, u8 *, const u8 *))
{
    ...
    while ((nbytes = walk.nbytes)) {
        ...
        do {
            ...
            wsrc += DES3_EDE_BLOCK_SIZE;
            wdst += DES3_EDE_BLOCK_SIZE;
        } 
        while ((nbytes -= DES3_EDE_BLOCK_SIZE) >= DES3_EDE_BLOCK_SIZE);
...
static int moto_ecb_des3_ede_encrypt(struct blkcipher_desc *desc,
        struct scatterlist *dst, struct scatterlist *src,
        unsigned int nbytes)
{
    return moto_ecb_des3_ede_operate(desc, dst, src, nbytes, 
            moto_des3_ede_encrypt);
}

static int moto_ecb_des3_ede_decrypt(struct blkcipher_desc *desc,
        struct scatterlist *dst, struct scatterlist *src,
        unsigned int nbytes)
{
    return moto_ecb_des3_ede_operate(desc, dst, src, nbytes, 
            moto_des3_ede_decrypt);
}

static int moto_cbc_des3_ede_encrypt_segment(struct blkcipher_desc *desc,
        struct blkcipher_walk *walk,
        struct crypto_blkcipher *tfm)
{
    ...
    do {
        moto_crypto_xor(iv, src, DES3_EDE_BLOCK_SIZE);
        moto_des3_ede_encrypt(crypto_blkcipher_tfm(tfm), dst, iv);
        memcpy(iv, dst, DES3_EDE_BLOCK_SIZE);

        src += DES3_EDE_BLOCK_SIZE;
        dst += DES3_EDE_BLOCK_SIZE;
    } while ((nbytes -= DES3_EDE_BLOCK_SIZE) >= DES3_EDE_BLOCK_SIZE);

    return nbytes;
}

static int moto_cbc_des3_ede_encrypt_inplace(struct blkcipher_desc *desc,
        struct blkcipher_walk *walk,
        struct crypto_blkcipher *tfm)
{
    unsigned int nbytes = walk->nbytes;
    ...
    do {
        moto_crypto_xor(src, iv, DES3_EDE_BLOCK_SIZE);
        moto_des3_ede_encrypt(crypto_blkcipher_tfm(tfm), src, src);
        iv = src;

        src += DES3_EDE_BLOCK_SIZE;
    } while ((nbytes -= DES3_EDE_BLOCK_SIZE) >= DES3_EDE_BLOCK_SIZE);

    memcpy(walk->iv, iv, DES3_EDE_BLOCK_SIZE);

    return nbytes;
}

static int moto_cbc_des3_ede_encrypt(struct blkcipher_desc *desc,
        struct scatterlist *dst, struct scatterlist *src,
        unsigned int nbytes)
{
    struct blkcipher_walk walk;
    ...
    while ((nbytes = walk.nbytes)) {
        if (walk.src.virt.addr == walk.dst.virt.addr)
            nbytes = moto_cbc_des3_ede_encrypt_inplace(desc, &walk,
                    tfm);
        else
            nbytes = moto_cbc_des3_ede_encrypt_segment(desc, &walk,
                    tfm);
        err = blkcipher_walk_done(desc, &walk, nbytes);
    }
...
static int moto_cbc_des3_ede_decrypt_segment(struct blkcipher_desc *desc,
        struct blkcipher_walk *walk,
        struct crypto_blkcipher *tfm)
{
    unsigned int nbytes = walk->nbytes;
    ...
    do {
        moto_des3_ede_decrypt(crypto_blkcipher_tfm(tfm), dst, src);
        moto_crypto_xor(dst, iv, DES3_EDE_BLOCK_SIZE);
        iv = src;
...
static int moto_cbc_des3_ede_decrypt_inplace(struct blkcipher_desc *desc,
        struct blkcipher_walk *walk,
        struct crypto_blkcipher *tfm)
{
    unsigned int nbytes = walk->nbytes;
    u8 *src = walk->src.virt.addr;
    u8 last_iv[DES3_EDE_BLOCK_SIZE];

    /* Start of the last block. */
    src += nbytes - (nbytes & (DES3_EDE_BLOCK_SIZE - 1)) 
	          - DES3_EDE_BLOCK_SIZE;
    memcpy(last_iv, src, DES3_EDE_BLOCK_SIZE);

    for (;;) {
        moto_des3_ede_decrypt(crypto_blkcipher_tfm(tfm), src, src);
        nbytes -= DES3_EDE_BLOCK_SIZE;
        if (nbytes < DES3_EDE_BLOCK_SIZE)
            break;
        moto_crypto_xor(src, src - DES3_EDE_BLOCK_SIZE, 
                DES3_EDE_BLOCK_SIZE);
        src -= DES3_EDE_BLOCK_SIZE;
    }

    moto_crypto_xor(src, walk->iv, DES3_EDE_BLOCK_SIZE);
    memcpy(walk->iv, last_iv, DES3_EDE_BLOCK_SIZE);

    return nbytes;
}

static int moto_cbc_des3_ede_decrypt(struct blkcipher_desc *desc,
        ...
        unsigned int nbytes)
{
    ...
    while ((nbytes = walk.nbytes)) {
        if (walk.src.virt.addr == walk.dst.virt.addr)
            nbytes = moto_cbc_des3_ede_decrypt_inplace(desc, &walk,
                    tfm);
        else
            nbytes = moto_cbc_des3_ede_decrypt_segment(desc, &walk,
                    tfm);
        err = blkcipher_walk_done(desc, &walk, nbytes);
    }
...
static struct crypto_alg moto_ecb_des3_ede_alg = {
        ...
        .cra_ctxsize    = sizeof(struct moto_des3_ede_ctx),
        ...
        .cra_u          = { 
                .blkcipher = {
                        ...
                        .setkey         = moto_des3_ede_setkey,
                        .encrypt        = moto_ecb_des3_ede_encrypt,
                        .decrypt        = moto_ecb_des3_ede_decrypt 
                } 
        }
};

static struct crypto_alg moto_cbc_des3_ede_alg = {
        .cra_name       = "cbc(des3_ede)",
        .cra_driver_name= "moto-des3-cbc",
        .cra_priority   = 1000,
        .cra_flags      = CRYPTO_ALG_TYPE_BLKCIPHER,
        .cra_type       = &crypto_blkcipher_type,
        .cra_blocksize  = DES3_EDE_BLOCK_SIZE,
        ...
        .cra_u          = { 
                .blkcipher = {
                        .min_keysize    = DES3_EDE_KEY_SIZE,
                        .max_keysize    = DES3_EDE_KEY_SIZE,
                        .ivsize         = DES3_EDE_IV_SIZE,
                        .setkey         = moto_des3_ede_setkey,
                        .encrypt        = moto_cbc_des3_ede_encrypt,
                        .decrypt        = moto_cbc_des3_ede_decrypt 
                } 
        }
};
...
void moto_tdes_finish(void)
{
    ...
    err = crypto_unregister_alg(&moto_ecb_des3_ede_alg);
    printk (KERN_INFO "ecb(des3) unregister result: %d\n", err);
    err = crypto_unregister_alg(&moto_cbc_des3_ede_alg);
    printk (KERN_INFO "cbc(des3) unregister result: %d\n", err);
